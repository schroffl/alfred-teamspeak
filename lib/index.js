...
class Alfred extends EventedStack {
     ...
     * @author      iHDeveloper
     * 
     * @return      {Promise} A promise that resolves when the list successfully loaded
     *                        rejects when the list didn't recieve yet.
     */
    clientsList() {
        return new Promise((resolve, reject) => {
            let users = null;
            this.query.send("clientlist").then(res => {
                let cids = res.cid;
                let clids = res.clid;
                let dbids = res["client_database_id"];
                let names = res["client_nickname"];
                let types = res["client_type"];
                users = new Array();
                for (const index in clids) {
                    let clid = clids[index];
                    let cid = cids[index];
                    let dbid = dbids[index];
    ...
    /**
     * Manage the channels in the bot server
     * 
     * @author      iHDeveloper
     */
    channel() {
        let object = {
             ...
             * @return      {Promise} A promise that resolves when the channel moved
             *                        rejected when the channel failed to move
             */
